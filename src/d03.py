        ...
        for r in re.finditer(r'\d+', line):
            ...
            for span in spans:
                ...
                for x in range(start-1, end+1):
                    top, bot = (i-1, x), (i+1, x)
                    value = adj.get(top)
                    if value: adj.update({top: value + [num]})
                    else: adj.update({top: [num]})

                    value = adj.get(bot)
                    if value: adj.update({bot: value + [num]})
                    else: adj.update({bot: [num]})


                left, right = (i, start-1), (i, end)
                value = adj.get(left)
                if value: adj.update({left: value + [num]})
                else: adj.update({left: [num]})

                value = adj.get(right)
                if value: adj.update({right: value + [num]})
                else: adj.update({right: [num]})
